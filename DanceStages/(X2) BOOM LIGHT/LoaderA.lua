...
local t = Def.ActorFrame{}
	...
	function DSMap(MapCode, _CM, X, Y, Z, rX, rY, rZ, Zo, ModelTime, sX, sY, sZ, bobX, bobY, bobZ, bobEOF, bobPeriod, Diffuse)
		t[#t + 1] =
			...
			OnCommand = function(self)
				if (MapCode == "Map_D" or MapCode == "Map_F3" or MapCode == "Map_C2") and GetUserPref("BoomSync") == "BPM Sync" then
			...
			Def.Model {
				...
				OnCommand = function(self)
					...
					if MapCode == "Map_F3" or MapCode == "Map_C3" then
						self:blend("BlendMode_Add")
					end
					if MapCode == "Map_C2" then
						self:rate(-1)
					end
					if MapCode == "Map_D" or MapCode == "Map_C2" then self:position(0.6) end
				end
			}
		}
	end
	...
	DSMap("Map_A",2)
	DSMap("Map_B",2,0,0,0,0,0,0,0,0,0,50)
